...
| **3** | Aug 10–21 | **Journals & payments** (D4 — heaviest gap) | Gen/payment/cash-receipt journals, applying entries, bank rec, reversals | Payment journal → apply to invoice; cash receipt → apply; undo/reverse; **bank reconciliation** — trace each into the G/L. **Client:** Masha monthly cycle (§11) | Show a posted bank rec + applied entries → **🔴→🟢** | ◐ *(started 15 Jul — AP payment + AR receipt cycles done solo w/ apply; fictitious vendor+customer built; deep VAT-Posting-Setup detour captured. Playbook Parts 1–3 written. **Still: reverse/unapply + bank rec** = Parts 4–5)* |
